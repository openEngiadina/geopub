(* This file has been taken from the ocaml-stemmer repository
   ...
   public domain. See the original header:
*)
...
let word_size word =
  ...
  let rec aux idx count state =
    if idx < wordlen then
      ...
      match state with
      | 0 -> if is_vowel word.[idx] then call count 1 else call count 2
      | 1 -> if is_vowel word.[idx] then call count 1 else call (succ count) 2
      | 2 ->
